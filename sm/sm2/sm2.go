...
var generateRandK = GenerateRandK
...
func GenerateRandK(rand io.Reader, c sm2curve.Curve) (k *big.Int) {
	params := c.Params()
	two := big.NewInt(2)
	b := make([]byte, params.BitSize/8+8)//为了保证位数为8的整数倍
	_, err := io.ReadFull(rand, b)//读取一个随机数填充b
	if err != nil {
		return
	}
	k = new(big.Int).SetBytes(b)//将b中间换成int
...
func getZById(pub *PublicKey, id []byte) []byte {
	...
	//h := sm3.New()
	hash := sm3.SumSM3(z)//使用SM3封装的杂凑函数
	return hash[:]
}
