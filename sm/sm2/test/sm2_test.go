...
func TestSignAndVerWithDigest(t *testing.T) {
	...
	copy(m, sm2.GetZ(&priv.PublicKey))
	copy(m[32:], msg)
	digest := sm3.SumSM3(m)
	r, s, err := sm2.SignWithDigest(rand.Reader, priv, digest[:])
	if err != nil {
		t.Errorf("sign with digest failed:%s", err)
		return
	}

	if !sm2.VerifyWithDigest(&priv.PublicKey, digest[:], r, s) {
...
func BenchmarkSignWithDigest(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = sm2.SignWithDigest(rand.Reader, priv, hashed)
	}
}
...
func BenchmarkVerifyWithDigest(b *testing.B) {
	...
	r, s, _ := sm2.SignWithDigest(rand.Reader, priv, hashed)

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sm2.VerifyWithDigest(&priv.PublicKey, hashed, r, s)
	}
}
...
func TestDecrypt(t *testing.T) {
	...
	priv := sm2.PrivateKey{sm2.PublicKey{sm2curve.P256(), pkx, pky, nil}, sk, nil}

	plain, err := sm2.Decrypt(c, &priv)
	if err != nil {
		t.Errorf("dec err:%s", err)
		return
	}
...
//func TestRandomValueGen(t *testing.T) {
//	suite := bn256.NewSuite()
//	n := 7
//	th := 3
//	nodes := randomValueGen(n, th, suite)
//	for i := range nodes{
//		fmt.Println(nodes[i].priKey)
//	}
//}
